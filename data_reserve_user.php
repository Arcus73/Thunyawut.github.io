      ...
      <div class="container">
        <div class="row">
          <div class="col-md-12"> <br>
            ...
            <?php
                    // คิวรีข้อมูลทั้งหมดเพื่อแสดงในตาราง
                    require_once 'config/db.php';
                    if (isset($_GET['q'])) {
                        $stmt = $conn->prepare("SELECT * FROM football_field WHERE date_reserve = ?");
                        $stmt->execute([$_GET['q']]);
                    } else {
                        $stmt = $conn->prepare("SELECT * FROM football_field ORDER BY date_reserve DESC, start_time ASC");
                        $stmt->execute();
                    }
